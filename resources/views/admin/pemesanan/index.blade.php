    ...
    <div class="container-fluid">
        <div class="row gap-2 justify-content-center">
            ...
            <div class="mb-4 mx-3 col-md-3 d-flex flex-column align-items-center justify-content-center py-4 gap-2 text-white card shadow-md" style="background-color: #949494;">
                ...
                <div class="d-flex flex-row align-items-center justify-content-center gap-2 fs-4">
                    <i class="fa-solid fa-money-check-dollar"></i>
                    <span>Rp.{{ number_format($totalFromCash) }}</span>
                </div>
            </div>
            <div class="mb-4 mx-3 col-md-3 d-flex flex-column align-items-center justify-content-center py-4 gap-2 text-white card shadow-md" style="background-color: #949494;">
                ...
                <div class="d-flex flex-row align-items-center justify-content-center gap-2 fs-4">
                    <i class="fa-solid fa-money-check-dollar"></i>
                    <span>Rp.{{ number_format($totalFromQris) }}</span>
                </div>
            </div>
        </div>
        ...
        <table class="table table-bordered border-dark align-middle">
            ...
            <tbody>
                ...
                <tr class="text-center">
                    ...
                    <td>{{ $d->name }}</td>
                    <td data-bs-toggle="modal" data-bs-target="#updateModalTableNumber{{ $d->id }}" class="cursor-pointer">{{ $d->table_number }}</td>
                    <td class="text-start">
                    ...
                    <td>
                        <div data-bs-toggle="modal" data-bs-target="#updateStatusPayment{{ $d->id }}" class="text-capitalize cursor-pointer badge 
                        @if($d->payment_method == 'cash') bg-success 
                        @elseif($d->payment_method == 'qris') bg-danger 
                        @endif">
                            {{ $d->payment_method }}
                        </div>
                    </td>
                ...
                <!-- Modal Payment -->
                <div class="modal fade mt-10" id="updateStatusPayment{{ $d->id }}" tabindex="-1"
                    aria-labelledby="updateStatusPaymentLabel{{ $d->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <form action="{{ route('pemesanan.updatePayment', $d->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="updateStatusPaymentLabel{{ $d->id }}">
                                        Ubah Payment Method - #{{ $d->id }} - {{ $d->name }}
                                    </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Tutup"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label for="payment_method-{{ $d->id }}">Pilih Method</label>
                                        <select name="payment_method" id="payment_method-{{ $d->id }}" class="form-control" required>
                                            <option value="cash" {{ $d->payment_method == 'cash' ? 'selected' : '' }}>Cash</option>
                                            <option value="qris" {{ $d->payment_method == 'qris' ? 'selected' : '' }}>Qris</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="modal-footer">
                ...
                <!-- Modal Table Number -->
                <div class="modal fade mt-10" id="updateModalTableNumber{{ $d->id }}" tabindex="-1"
                    aria-labelledby="updateModalTableNumberLabel{{ $d->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <form action="{{ route('pemesanan.updateTableNumber', $d->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="updateModalTableNumberLabel{{ $d->id }}">
                                        Ubah Nomor Meja - #{{ $d->id }} - {{ $d->name }}
                                    </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Tutup"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label for="table_number-{{ $d->id }}">Nomor Meja</label>
                                        <input type="number" name="table_number" id="table_number-{{ $d->id }}" class="form-control" required />
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
